...
slavesfile=$HOME/slaves.txt
hostsfile=$HOME/hosts.txt
rm -f $hostsfile
...
# for size in 2 4 8 16 32 64 128 256 512 1024 2048 4096 8192 16384 ; do
	...
	restartfile=$datadir/cdhit/$filename.restart
	...
	echo "running CD hit "
	start=$SECONDS
	echo "$cdhitdir/cd-hit-para.pl -i $input -o $output -c 0.95 -n 9 -M 230000 -T 32 -aS 0.8 -G 1 -g 1 --B $hostsfile --P cd-hit-est"
	$cdhitdir/cd-hit-para.pl -i $input -o $output -c 0.95 -n 9 -M 230000 -T 32 -aS 0.8 -G 1 -g 1 --B $hostsfile --P cd-hit-est
	# --R $restartfile
